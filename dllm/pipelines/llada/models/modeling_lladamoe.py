...
_CONFIG_FOR_DOC = "LLaDAMoEConfig"
...
class LLaDAMoERotaryEmbedding(nn.Module):
    def __init__(
        ...
        scaling_factor=1.0,
        rope_type="default",
        config: Optional[LLaDAMoEConfig] = None,
    ):
        super().__init__()
...
class LLaDAMoEAttention(nn.Module):
    ...
    def __init__(self, config: LLaDAMoEConfig, layer_idx: Optional[int] = None):
        super().__init__()
        self.config = config
...
class LLaDAMoEDecoderLayer(nn.Module):
    def __init__(self, config: LLaDAMoEConfig, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size
...
class LLaDAMoEModel(LLaDAMoEPreTrainedModel):
    ...
    def __init__(self, config: LLaDAMoEConfig):
