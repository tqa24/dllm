...
accelerate launch --num_processes "${num_gpu}" dllm/pipelines/llada/eval.py \
    ...
    --model_args "pretrained=${model_name_or_path},max_new_tokens=3,steps=3,block_length=3,cfg=0.0,right_shift_logits=${right_shift_logits}"

accelerate launch --num_processes "${num_gpu}" dllm/pipelines/llada/eval.py \
    --tasks mmlu_pro --num_fewshot 0 ${common_args} \
    --model_args "pretrained=${model_name_or_path},max_new_tokens=256,steps=256,block_length=256,cfg=0.0,right_shift_logits=${right_shift_logits}"
...
accelerate launch --num_processes "${num_gpu}" dllm/pipelines/llada/eval.py \
    ...
    --model_args "pretrained=${model_name_or_path},max_new_tokens=512,steps=512,block_length=512,cfg=0.0,right_shift_logits=${right_shift_logits}"

accelerate launch --num_processes "${num_gpu}" dllm/pipelines/llada/eval.py \
    --tasks gpqa_diamond_generative_n_shot --num_fewshot 5 ${common_args} \
    --model_args "pretrained=${model_name_or_path},max_new_tokens=64,steps=64,block_length=64,cfg=0.0,right_shift_logits=${right_shift_logits}"
...
accelerate launch --num_processes "${num_gpu}" dllm/pipelines/llada/eval.py \
    ...
    --model_args "pretrained=${model_name_or_path},max_new_tokens=1024,steps=1024,block_length=1024,cfg=0.0,right_shift_logits=${right_shift_logits}"
